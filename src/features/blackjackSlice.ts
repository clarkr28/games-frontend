  ...
  reducers: {
    ...
    hitDealer: (state) => {
      ...
      if (state.dealerHandValue >= 17) {
        state.betPool = getBlackjackWinnings(
          state.playerHandValue,
          state.playerHand.length,
          state.dealerHandValue,
          state.betPool,
        );
    ...
    finalizeBet: (state, action: PayloadAction<number>) => {
      if (
        ...
        action.payload > 0 &&
        action.payload <= state.playerBank
      ) {
...
export const selectBlackjackStatus = (state: RootState) =>
  state.blackjack.gameStatus;
export const selectBlackjackDealerHand = (state: RootState) =>
  state.blackjack.dealerHand;
export const selectBlackjackDealerHandValue = (state: RootState) =>
  state.blackjack.dealerHandValue;
export const selectBlackjackPlayerHand = (state: RootState) =>
  state.blackjack.playerHand;
export const selectBlackjackPlayerHandValue = (state: RootState) =>
  state.blackjack.playerHandValue;
export const selectBlackjackPlayerBank = (state: RootState) =>
  state.blackjack.playerBank;
export const selectBlackjackBetPool = (state: RootState) =>
  state.blackjack.betPool;
