...
export const lifeSlice = createSlice({
  ...
  reducers: {
    advanceGeneration: (state) => {
      ...
      [state.board, state.liveCellKeys] = makeNextGeneration(
        state.board,
        state.liveCellKeys,
      );
      if (state.presetSelection !== null && state.hoverPoint) {
        ...
        [state.boardWithHover] = applyPresetToBoard(
          state.board,
          state.hoverPoint,
    ...
    toggleCell: (state, action: PayloadAction<Point>) => {
      if (state.presetSelection !== null) {
        ...
        [state.board, newEncodedCells] = applyPresetToBoard(
          state.board,
          action.payload,
          state.presetSelection,
          false,
        ...
        [state.board, state.liveCellKeys] = toggleBoardCell(
          state.board,
          state.liveCellKeys,
          action.payload,
        );
...
export const selectLifeBoard = (state: RootState) =>
  state.life.boardWithHover || state.life.board;
