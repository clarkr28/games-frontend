 ...
 */
export function toggleBoardCell(
  board: LifeCellState[][],
  liveCellKeys: string[],
  point: Point,
  ...
  if (point.Y < board.length && point.X < board[0].length) {
    const newCellValue =
      board[point.Y][point.X] === LifeCellState.Alive
        ? LifeCellState.Dead
        : LifeCellState.Alive;
...
function cellLiveNeighbors(
  ...
  for (let i = x - 1; i <= x + 1; i++) {
    ...
    if (
      ...
      i >= 0 &&
      i < board[0].length &&
      board[y - 1][i] === LifeCellState.Alive
    ) {
      liveCount++;
    }
...
function processCellAndNeighbors(
  ...
  if (
    point.X < 0 ||
    point.X >= newBoard[0].length ||
    point.Y >= newBoard.length
  ) {
    return;
  }
  ...
  offsets.forEach((offset) =>
    processCell(
      { X: point.X + offset[1], Y: point.Y + offset[0] },
      oldBoard,
      newBoard,
      processedKeys,
      newLiveKeys,
...
function processCell(
  ...
  if (
    cell.X < 0 ||
    cell.X >= oldBoard[0].length ||
    cell.Y < 0 ||
    cell.Y >= oldBoard.length
  ) {
    return;
  }
  ...
  if (
    oldBoard[cell.Y][cell.X] === LifeCellState.Alive &&
    ...
    oldBoard[cell.Y][cell.X] === LifeCellState.Dead &&
    liveNeighbors === 3
  ) {
...
export function processBoardResize(
  board: LifeCellState[][],
  newSize: IRect,
): LifeCellState[][] {
  //
  ...
  if (pairedColDiff > 0) {
    ...
    for (let i = 0; i < board.length; i++) {
      board[i].push(
        ...Array<LifeCellState>(pairedColDiff).fill(LifeCellState.Dead),
      );
      board[i].unshift(
        ...Array<LifeCellState>(pairedColDiff).fill(LifeCellState.Dead),
      );
  ...
  if (rowDiff > 0) {
    ...
    for (let i = 0; i < rowDiff; i++) {
      board.push(
        Array<LifeCellState>(board[0].length).fill(LifeCellState.Dead),
      );
    }
...
export function applyPresetToBoard(
  ...
  for (let yInd = 0; yInd < presetData.height; yInd++) {
    ...
    for (let xInd = 0; xInd < presetData.width; xInd++) {
      ...
      if (
        xCoord >= 0 &&
        xCoord < board[0].length &&
        presetData.data[yInd][xInd] === LifeCellState.Alive
      ) {
        newBoard[yCoord][xCoord] = stateToSet;
        if (
          stateToSet === LifeCellState.Alive &&
          board[yCoord][xCoord] === LifeCellState.Dead
        ) {
          // if the cell was dead, we need to track a new encoded cell value for that cell
          newEncodedCells.push(
            encode({ X: xCoord, Y: yCoord }, board[0].length),
          );
        }
      }
    }
