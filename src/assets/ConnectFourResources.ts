...
export function isBoardFull(board: C4CellState[][]): boolean {
  ...
  return !board.some(
    (column) => column[column.length - 1] === C4CellState.Empty,
  );
}
...
export function matchesHorizontal(
  board: C4CellState[][],
  startX: number,
  startY: number,
): Point[] {
...
export function calculateStatus(
  ...
  if (winningCells) {
    ...
    return board[winningCells[0].X][winningCells[0].Y] === C4CellState.Black
      ? C4GameStatus.WinBlack
      : C4GameStatus.WinRed;
  }
