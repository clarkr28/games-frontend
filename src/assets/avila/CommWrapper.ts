...
import {
  ...
  IEndTurnRequest,
  IEndTurnResponse,
  IJoinRoomHostResponse,
  IJoinRoomPlayerResponse,
  IJoinRoomRequest,
  IPlacedTileRequest,
  IPlacedTileResponse,
  IReconnectRequest,
...
export class CommWrapper {
  ...
  public static opponentPlacedTileCallback?: (data: IPlacedTileData) => void =
    undefined;
  public static opponentEndTurnCallback?: (data: IEndTurnData) => void =
    undefined;
  public static serverConnectedCallback?: (isConnected: boolean) => void =
    undefined;
  ...
  private static CreateSocket() {
    ...
    const timeoutInterval = parseInt(
      process.env.REACT_APP_AVILA_FORCE_RECONNECT_MS ?? "0",
    );
    if (timeoutInterval) {
      CommWrapper.forceReconnectTimeoutId = window.setTimeout(
        CommWrapper.ForceReconnect,
        timeoutInterval,
      );
    }
  }
  ...
  private static AddListeners() {
    ...
    CommWrapper.socket.addEventListener(
      "message",
        ...
        if (message) {
          switch (message.type) {
            ...
            case "StartGameResponse":
              const startGameData: IStartGameData = JSON.parse(
                message.data,
              ) as IStartGameData;
